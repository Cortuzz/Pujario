...
const uint8_t permutation[] = { 151,160,137,91,90,15,
...
{
	auto xi = static_cast<int>(x) & 255;								// Calculate the "unit cube" that the point asked will be located in
	auto yi = static_cast<int>(y) & 255;								// The left bound is ( |_x_|,|_y_|,|_z_| ) and the right bound is that
	auto zi = static_cast<int>(z) & 255;								// plus 1.  Next we calculate the location (from 0.0 to 1.0) in that cube.

	const auto xf = x - static_cast<int>(x);								// We also fade the location to smooth the result.
	const auto yf = y - static_cast<int>(y);
	const auto zf = z - static_cast<int>(z);

	const auto u = Fade(xf);
	const auto v = Fade(yf);
	const auto w = Fade(zf);
	
	auto a  = permutation[xi] + yi;							// This here is Perlin's hash function.  We take our x value (remember,
	auto aa = permutation[a] + zi;								// between 0 and 255) and get a random value (from our permutation[] array above) between
	auto ab = permutation[a + 1] + zi;							// 0 and 255.  We then add y to it and plug that into permutation[], and add z to that.
	auto b  = permutation[xi + 1] + yi;						// Then, we get another random value by adding 1 to that and putting it into permutation[]
	auto ba = permutation[b] + zi;								// and add z to it.  We do the whole thing over again starting with x+1.  Later
	auto bb = permutation[b + 1] + zi;							// we plug aa, ab, ba, and bb back into permutation[] along with their +1's to get another set.
	...
	auto x1 = Lerp(Gradient(permutation[aa], xf, yf, zf), Gradient(permutation[ba], xf - 1, yf, zf), u);
	auto x2 = Lerp(Gradient(permutation[ab], xf, yf - 1, zf), Gradient (permutation[bb], xf - 1, yf-1, zf), u);
	const auto y1 = Lerp(x1, x2, v);

	x1 = Lerp(Gradient (permutation[aa + 1], xf, yf, zf - 1), Gradient (permutation[ba + 1], xf-1, yf, zf - 1), u);
	x2 = Lerp(Gradient (permutation[ab + 1], xf, yf - 1, zf - 1),Gradient (permutation[bb + 1], xf - 1, yf - 1, zf - 1),u);
...
{
	const auto h = hash & 15; // Take the hashed value and take the first 4 bits of it (15 == 0b1111)
	const auto u = h < 8 /* 0b1000 */ ? x : y;
